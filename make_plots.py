...
def plot_confusion_matrix(ckpt_path: str, out_path: str,
    ...
    # En COCO (81 clases) la CM es ilegible entera → filtra a las top-K
    if top_k and top_k < cm.shape[0]:
        sums    = cm.sum(dim=1)                                # freq por clase real
        top_idx = sorted(sums.argsort(descending=True)[:top_k].tolist())
        cm_n    = cm_n[np.ix_(top_idx, top_idx)]
    ...
    ax.set_title(f"Confusion matrix{title_extra}  ·  epoch={ep}, mIoU={miou:.4f}")
...
def main() -> None:
    ...
    cm.add_argument("--top-k",     type=int, default=None,
                    help="Solo plotea las K clases con más píxeles en GT "
                         "(útil con COCO/81 cls)")
